...
#define bit_size 4 // update with required bit size
#define input_len 5 // update with array length
using namespace std;  
vector<int> input() {
    vector<int>temp;   
    for(ll i = 0; i < input_len; i++) {
        ll x; 
        cin >> x;  
        temp.push_back(x);
    }
...
ll checksum(vector<int>& data) {
    ...
    for(ll i = 0; i<bit_size; i++) {
        if(bit[i]) x += (1ll << i); // or x += pow(2,i);
        if(bit[i+bit_size]) y += (1ll << i); // or x += pow(2,i);
    }
    x += y;   
